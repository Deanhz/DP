...
def train(mnist):
    ...
    weights1 = tf.Variable(tf.truncated_normal(
        [INPUT_NODE, LAYER1_NODE], stddev=0.1), name="weights1")
    biases1 = tf.Variable(tf.constant(
        0.1, shape=[LAYER1_NODE]), name="biases1")
    weights2 = tf.Variable(tf.truncated_normal(
        [LAYER1_NODE, OUTPUT_NODE], stddev=0.1), name="weights2")
    biases2 = tf.Variable(tf.constant(
        0.1, shape=[OUTPUT_NODE], name="biases2"))
